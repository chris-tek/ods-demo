...
resource "aws_s3_bucket" "create_s3_buckets" {
  ...
  tags = {
    Source = "Terraform"
    Name   = "chris_ijames_test_bucket_${each.value}"
    Owner  = "Chris Ijames"
    POC    = "Chris Ijames"
    System = "Chris Ijames"
  }
...
resource "aws_s3_bucket_notification" "bucket_notifications_for_my_test_bucket" {
  ...
  queue {
    ...
    queue_arn     = "${var.chris_test_s3_event_notification_queue_arn}"
  ...
  queue {
    ...
    queue_arn     = "${var.chris_test_s3_event_notification_queue_2_arn}"
    events        = ["s3:ObjectCreated:*"]
